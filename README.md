...
When asked to use **TensorFlow** instead of PyTorch,  only to compare whether the same architectures, losses and optimizers were used, the models generally used larger batch sizes and more epochs, prioritizing stability over speed. TensorFlow implementations frequently incorporated Dice Loss alongside Binary Cross-Entropy to handle class imbalance, whereas PyTorch versions relied primarily on Binary Cross-Entropy or BCEWithLogitsLoss. Additionally, TensorFlow models often included Batch Normalization in the convolutional blocks to improve convergence. These differences led to TensorFlow models having longer training times but more robust performance on imbalanced data, while PyTorch models trained faster with lower batch sizes and fewer epochs, sometimes at the expense of stability. Despite these variations, the core architectures and parameter counts remained consistent across frameworks.

2. Model Architecture and Hyper parameter differences when asked to use Tensorflow:
...
**Table**: Comparison of Features Across Different LLM-based Models, when asked to use Tensorflow instead of Pytorch.
