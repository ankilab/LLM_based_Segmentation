...
for i, model in enumerate(train_models):
    ...
    # --- add initial-epoch marker dot ---
    e0 = num_epochs[model]
    y0 = train_losses.iloc[i].dropna().iloc[e0 - 1]  # (use val_losses in validation loops)
    axes[0].scatter(e0, y0, s=80, color=color_map[model], edgecolors='none', zorder=5)
...
for i, model in enumerate(val_models):
    ...
    e0 = num_epochs[model]
    y0 = val_losses.iloc[i].dropna().iloc[e0 - 1]
    axes[1].scatter(e0, y0, s=80, color=color_map[model], edgecolors='none', zorder=5)
...
plt.savefig(f"{save_path}all_model_losses_baseline_total_BAGLS_120_marked_epochs.png", dpi=600)
...
for i, model in enumerate(train_models):
    ...
    axes[0].plot(epochs, train_losses.iloc[i].dropna(), label=model, color=color_map[model])
    e0 = num_epochs[model]
    y0 = train_losses.iloc[i].dropna().iloc[e0 - 1]
    axes[0].scatter(e0, y0, s=80, color=color_map[model], edgecolors='none', zorder=5)
axes[0].set_title('BAGLS Dataset', fontsize=16)
axes[0].set_xlabel('Epoch', fontsize=14)
axes[0].set_ylabel('Training Loss', fontsize=14)
...
plt.savefig(f"{save_path}all_model_losses_logarithmic_baseline_total_BAGLS_120_marked_epochs.png", dpi=600)
#plt.show()
