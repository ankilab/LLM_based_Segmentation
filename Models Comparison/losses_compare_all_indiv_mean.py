...
def plot_models(train_path, val_path, year, save_dir):
    ...
    plt.savefig(os.path.join(save_dir, f"{year}_all_model_losses_MYOMA_mean_linear.png"), dpi=600)
    plt.close()
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.93])
    plt.savefig(os.path.join(save_dir, f"{year}_all_model_losses_MYOMA_mean_log.png"), dpi=600)
    plt.close()
...
def plot_combined(train_2024, val_2024, train_2025, val_2025, save_dir, log=False):
    ...
    plt.savefig(os.path.join(save_dir, f"2024_vs_2025_all_model_losses_MYOMA_{fname}"), dpi=600)
    plt.close()
