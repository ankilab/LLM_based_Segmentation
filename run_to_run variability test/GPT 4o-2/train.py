...
def train_model(model, train_loader, val_loader, device, save_path, epochs=20, lr=1e-3):
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for images, masks, _ in val_pbar:
                ...
                mean_batch_dice = batch_dice.mean().item()
                batch_dices.append(mean_batch_dice)

                val_pbar.set_postfix(loss=loss.item(), dice=mean_batch_dice)

        avg_val_loss = val_loss / len(val_loader)
        val_losses.append(avg_val_loss)
        val_dice_epochwise.append(batch_dices)
    ...
    # Save validation dice scores (mean per batch per epoch)
    max_batches = max(len(b) for b in val_dice_epochwise)
    padded_dice = [b + [np.nan] * (max_batches - len(b)) for b in val_dice_epochwise]
...
def test_model(model, test_loader, device, save_path):
    ...
    with torch.no_grad():
        ...
        for images, masks, filenames in test_pbar:
            ...
            batch_mean_dice = dice.mean().item()
            all_dice.append(batch_mean_dice)
